...
if (Test-Path Burp-Suite-Pro.jar){
    ...
    if (((Get-Item Burp-Suite-Pro.jar).length/1MB) -lt 500 ){
        echo "`n`t`tFiles Seems to be corrupted `n`t`tDownloading Burp Suite Professional v2022.9 ...."
    ...
    echo "`n`t`tDownloading Burp Suite Professional v2022.9 ...."
    wget "https://portswigger-cdn.net/burp/releases/download?product=pro&version=2022.9&type=jar" -O "Burp-Suite-Pro.jar"
    echo "`nBurp Suite Professional is Downloaded.`n"
}
